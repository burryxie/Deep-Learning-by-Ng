...
%                   otherwise.
    ...
    cost = 1/numExamples * sum(sum((weightMatrix * featureMatrix -patches).^2)) + lambda * sum(sqrt(sum(featureMatrix .^ 2,2)) + epsilon)...
        + gamma * sum(sum(weightMatrix.^2));
    
    
    grad = 1/numExamples * (2 * weightMatrix * featureMatrix *featureMatrix' - 2 * patches * featureMatrix') + 2 * gamma * weightMatrix;
